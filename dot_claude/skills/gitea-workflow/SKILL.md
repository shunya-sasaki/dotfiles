---
name: gitea-workflow
description: When you're asked to work with Gitea, you MUST use this skill.
---

# Gitea Workflow

This skill provides instructions for Gitea repositories using the official `tea` CLI.

## Create Issue

When you're asked to create an issue, use this skill.
...
2. If the repository context is needed, inspect relevant files in the repository
   to understand the current implementation, configuration, documentation, or behavior.
3. Create the issue title using the **Conventional Commit** style.
4. If `.gitea/ISSUE_TEMPLATE/bug_report.md` or `.gitea/ISSUE_TEMPLATE/feature_request.md`
   (or any issue template file) exists in the repository, use it as the description template.
...
7. Get labels for the project with `tea label list` and select one for this issue.
8. Use **Read tool** to get the contents of temporary Markdown file.
9. Run `tea issue create --title <title> --description <description> --labels <label>`
   to create the issue on Gitea server.
...
3. If `.gitea/pull_request_template.md` (or any PR template file) exists in
...
6. Get labels for the project with `tea label list` and select one for this PR.
7. Use **Read tool** to read the description of the PR from the Markdown file.
8. Run `tea pr create --title <title> --description $description --labels <label> -b <target-branch>`
   to create a PR on the Gitea server.
...
#### Create gitea directory in the project

1. If `.gitea` directory does not exist, create it.

#### Create template for pull request

1. Check the file `.gitea/pull_request_template.md` exist.
...
_.gitea/pull_request_template.md_
...
1. If `.gitea/ISSUE_TEMPLATE` does not exist in the project, create it.
2. If `.gitea/ISSUE_TEMPLATE/bug_report.md` doesn't exist, create as the following:

_.gitea/ISSUE_TEMPLATE/bug_report.md_
...
3. If `.gitea/ISSUE_TEMPLATE/feature_request.md` doesn't exist, create as the following:
